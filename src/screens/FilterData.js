...
const FilterData = ({route, navigation}) => {
  ...
  // useEffect(() => {
  //   fetch(
  //     'https://dindayalupadhyay.smartcitylibrary.com/api/v1/books?order_by=created_at&limit=20&search=&genre=art&library_id=111&author=&publisher=&language=0&format=0',
  //   )
  //     .then(res => res.json())
  //     .then(respo => {
  //       /* setRecentBooks(respo.data); */
  //       setFilteredBooks(respo.data);
  //       /* setisLoaded(false); */
  //     });
  // }, [selectedGenre]);
  ...
  const handleSearchAuthor = text => {
    // console.log('Search Query:', text);
    setIsLoading(true);
    const filteredResults = authr.filter(item =>
      item.toLowerCase().includes(text.toLowerCase()),
    );
    // console.log('Filtered Results:', filteredResults);
  ...
  //     case 'Author':
  //        filteredResults = authr.filter((item) =>
  //     item.toLowerCase().includes(text.toLowerCase())
  //   );
  //   console.log('Filtered Author Results:', filteredResults);
  //   setSearchResultsAuthor(filteredResults);
  //   setIsLoading(false);
  //       break;
    ...
    if (searchQueryauthor.trim() !== '') {
      ...
      filteredBooksCopy = filterBybooks.filter(book => {
        const hasMatchingBookNameAuthor = book.authors.some(authr => {
          return (
            authr.first_name.toLowerCase().includes(queryauthor) ||
            authr.last_name.toLowerCase().includes(queryauthor)
          );
        });
        // console.log('hasMatchingBookNameAuthor', hasMatchingBookNameAuthor);
              ...
              <ScrollView
                ...
                <View
                  ...
                  <View
                    ...
                    <Picker
                      style={{marginTop: -10, marginStart: 5, color: '#5D6D7E'}}
...
// ---------------------------------------- using route getting data auther filter  =============================================
